...
PythonShellModule::PythonShellModule(const Handle& hcsn) : Module(hcsn)
{
	// Initialize Python.
...
std::string PythonShellModule::shellout(Request *req, std::list<std::string> args)
{
    ...
    PythonShell *sh = new PythonShell(_hcsn);
    sh->set_socket(con);
...
std::string PythonShellModule::do_eval(Request *req, std::list<std::string> args)
{
    ...
    PythonEval* pyev = PythonEval::get_python_evaluator(AtomSpaceCast(_hcsn->getAtomSpace()));
    pyev->begin_eval();
